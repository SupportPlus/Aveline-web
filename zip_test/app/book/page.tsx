...
const BOOK_SERVICES = [
  { id: "skin",         labelAr: "العناية بالبشرة",  labelEn: "Skin Care" },
  ...
  { id: "skin_boosters",labelAr: "إبر النضارة",        labelEn: "Skin Boosters" },
  { id: "dermatology",  labelAr: "الجلدية",            labelEn: "Dermatology" },
...
export default function BookPage() {
  ...
  const serviceLabel = BOOK_SERVICES.find((s) => s.id === serviceId)?.labelAr ?? serviceId;
  ...
  return (
    <>
      ...
      <main dir="rtl" className="min-h-screen bg-[var(--background)]">
        {/* Page header */}
        <section className="relative bg-[var(--background2)] py-28 overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-6">
            <Reveal>
              <Link href="/" className="inline-flex items-center gap-2 text-sm text-[var(--gold)] mb-8 hover:opacity-75 transition-opacity font-sans group">
                <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform duration-300" />
                الرئيسية
              </Link>
              <div className="flex items-center gap-3 mb-6">
                <span className="h-px w-8 bg-[var(--gold)]" />
                <span className="text-[var(--gold)] text-xs tracking-[0.3em] uppercase font-sans">حجز موعد</span>
              </div>
              ...
              <p className="mt-5 text-lg text-[var(--muted)] max-w-xl leading-relaxed font-sans">
                أكملي البيانات وسنتواصل معك فورًا عبر واتساب لتأكيد الحجز
              </p>
            </Reveal>
          </div>
        </section>
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  ...
                  <h2 className="text-3xl font-serif text-[var(--foreground)]">تم الإرسال</h2>
                  <p className="text-[var(--muted)] leading-relaxed font-sans">
                    تم فتح واتساب لتأكيد موعدك. سيتواصل معك فريقنا في أقرب وقت ممكن.
                  </p>
                  <button
                ...
                >
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-[var(--foreground)] font-sans" htmlFor="name">
                      الاسم <span className="text-[var(--gold)]">*</span>
                    </label>
                    <input
                      id="name"
                      required
                  ...
                  {/* Phone */}
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-[var(--foreground)] font-sans" htmlFor="phone">
                      رقم الجوال <span className="text-[var(--gold)]">*</span>
                    </label>
                    <input
                      id="phone"
                      required
                  ...
                  <div className="space-y-3">
                    <label className="text-sm font-semibold text-[var(--foreground)] font-sans">
                      الخدمة المطلوبة <span className="text-[var(--gold)]">*</span>
                    </label>
                    <motion.div
                    ...
                    >
                      {BOOK_SERVICES.map((s, i) => {
                        const isLast = i === BOOK_SERVICES.length - 1 && BOOK_SERVICES.length % 2 !== 0;
                        return (
                          <motion.button
                            key={s.id}
                            type="button"
                            ...
                            className={[
                              ...
                              isLast ? "col-span-2" : "",
                              serviceId === s.id
                                ? "bg-[var(--gold)] text-white border-[var(--gold)] shadow-[0_2px_16px_rgba(198,169,107,0.30)] font-semibold"
                  ...
                  {/* Notes */}
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-[var(--foreground)] font-sans" htmlFor="notes">
                      ملاحظات <span className="text-[var(--muted)] text-xs">(اختياري)</span>
                    </label>
                    <textarea
                      id="notes"
                      value={notes}
                  ...
                  <motion.button
                    ...
                    whileHover={name.trim() && phone.trim() && serviceId ? { scale: 1.01 } : {}}
                    whileTap={name.trim() && phone.trim() && serviceId ? { scale: 0.98 } : {}}
                    suppressHydrationWarning
                    className={[
                  ...
                  <p className="text-center text-xs text-[var(--muted)] font-sans">
                    أو اتصلي بنا مباشرة على{" "}
                    <a href="tel:+966551234567" className="font-semibold text-[var(--gold)] hover:opacity-80 transition inline-flex items-center gap-1">
                      <Phone size={11} />
